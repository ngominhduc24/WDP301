...
const UpdateStore = ({ open, oncancel, onOk, stores, id }) => {
  ...
  const onContinue = async () => {
    try {
      ...
      const res = await AdminServices.updateStores(stores._id, {
        ...storesData,
      })
      if (res?.isError) {
        return
  ...
  const column = [
    ...
    },
    {
      ...
      render: (text, record) => (
        <Input
          ...
          onChange={value =>
            handleChange(record._id, "location", value.target.value)
          }
        />
      ),
    },
    {
      ...
      render: (text, record) => (
        <Input
          ...
          onChange={e => handleChange(record._id, "phone", e.target.value)}
        />
      ),
    },
    {
      ...
      render: (text, record) => (
        <Input
          ...
          onChange={e => handleChange(record._id, "email", e.target.value)}
        />
      ),
    },
    // {
    ...
    },
    {
      ...
      render: (_, record) => (
        ...
        >
          {stateBody[record._id]?.status === "open"
            ? "Đang hoạt động"
            : "Dừng Hoạt Động"}
        </span>
      ),
